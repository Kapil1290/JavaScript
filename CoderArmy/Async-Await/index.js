...
async function ordering() {
    try {
    const res1 = await paymentOrder(orderDetails)
    const res2 = await preparingOrder(res1);
    const res3 = await pickOrder(res2);
    const res4 = await deliveryOrder(res3);
    console.log(res4);
...
async function user(){
    // const comment = await fetch("userComments");
    // const photo = await fetch("userPhote")
    // const userChat = await fetch("userChat")       // it all resolve one-by-one

    const [comment,photo,chats]=await Promise.all([fetch("userComment"), fetch("userPhoto"), fetch("userChats")])  // for parallel execution on the api's
}
